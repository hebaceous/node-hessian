...
// fixed UTF-8
BufferReader.prototype.nextUTF8String = function(length) {
    var total = 0
    while (length--) {
        var head = this.nextUInt8();
        // var mask = head & 0x80// 10000000
        // var mask = head & 0xE0// 11100000
        // var mask = head & 0xF0// 11110000
        // var mask = head & 0xF8// 11111000
        var bytes = 1;
        if ((head & 0x80) === 0x00) {
            bytes = 1;
...
Reader.prototype.readString = function(data) {
    ...
    var code = this.reader.nextUInt8();
    if (code >= 0 && code < 32) {
        return this.reader.nextUTF8String(code);
    } else if (code >= 0x30 && code <= 0x33) {
        this.reader.move(-1);
        var len = this.reader.nextUInt16BE() - 0x3000;
        return this.reader.nextUTF8String(len);
    } else if (code === 0x53) {
        var len = this.reader.nextUInt16BE();
        return this.reader.nextUTF8String(len);
    } else if (code === 0x52) {
        var len = this.reader.nextUInt16BE();
        return this.reader.nextUTF8String(len) + this.readString();
    }
};

var Long = new require('long');
...
Reader.prototype.readList = function(data) {
    ...
    if (code >= 0x70 && code <= 0x7f) {
        ...
        if (code <= 0x77) {
            type = this.readType();
            Object.defineProperty(rs, "__type__", {
                value: type,
                enumerable: false
            });
            len += 0x8;
